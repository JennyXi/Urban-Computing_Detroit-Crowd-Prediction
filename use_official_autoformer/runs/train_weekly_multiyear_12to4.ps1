...
# Multi-year weekly file (already exists in this repo)
$DataPath = "autoformer_weekly.csv"
...
$Dims = $Dims.Trim()
Write-Host "Detected dims (enc_in=dec_in=c_out) = $Dims"

Set-Location $AutoformerRoot

& $Py -u .\run.py `
  --is_training 1 `
  --model_id detroit_multiyear_weekly `
  --model Autoformer `
  --data custom `
  --root_path "$DataRoot" `
  --data_path "$DataPath" `
